...
from utils.datasetOrg import create_dataloader as create_dataloader_org
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    # Process 0
    if RANK in [-1, 0]:
        val_loader = create_dataloader_org(val_path, imgsz, batch_size // WORLD_SIZE * 2, gs, single_cls,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        LOGGER.info(('\n' + '%10s' * 8) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'deg' ,'labels', 'img_size'))
        if RANK in [-1, 0]:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--name', default='deg-0.9', help='save to project/name')
    ...
    opt.project = opt.cfg.replace(".yaml", "").replace(".cfg", "") 

    if "Use_Double_Head" in os.environ: 
        opt.name = 'Double_Head.' + opt.name
    return opt
