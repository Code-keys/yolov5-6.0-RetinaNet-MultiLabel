...
from utils.datasetOrg import create_dataloader
...
def run(data,
    ...
    names =  { k: v for k, v in enumerate( data["names"]  )} #  {k: v for k, v in enumerate(model.names if hasattr(model, 'names') else model.module.names)}
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        # Run NMS
        out = non_max_suppression( out_val, conf_thres, iou_thres, labels=lb, multi_label=True, agnostic=single_cls)
        dt[2] += time_sync() - t3
    ...
    maps = np.zeros(  ap_class.__len__() ) + map
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
